...
def find_final_module_path(path, data):
	if '_module' in data:
		if data['_module'] in data and '_module' in data[data['_module']] and isdir(f"{path}/{data['_module']}"):
			...
			return {'path' : f"{path}/{data['_module']}.py", 'data' : data, 'api_path' : ':'.join(f"{path}/{data['_module']}"[len('./api/'):].split('/'))}
	...
	def parse(self, client, data, headers, fileno, addr, *args, **kwargs):
		...
		if(module_to_load):
			...
			if import_result:
				...
				try:
					response = modules[module_to_load['path']].parser().process(f'api', client, module_to_load['data'], headers, fileno, addr, *args, **kwargs)
					if response:
...
with open('./secrets/obtain.life.secrets.json', 'r') as fh:
